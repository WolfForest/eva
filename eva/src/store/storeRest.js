export default {
  async rest(formData, searchFrom, restAuth, idDash) {
    let response = await fetch(`/api/makejob`, {
            ...
            let cycle = new Promise((resolve) => {
              ...
              let timeOut = setTimeout(async function tick() {
                ...
                if (responseGet != 200 && responseGet != 0) {
                  ...
                  // если прошёл
                  let dataEvents = await resEvents.json().then((res) => {
  ...
  async getGroups(restAuth) {
    let data = [];
    let response = await fetch(`/api/user/groups`).catch((error) => {
      restAuth.putLog(
  ...
  async getDashs(id, restAuth) {
    let data = [];
    let response = await fetch(`/api/group/dashs?id=${id}`).catch((error) => {
      restAuth.putLog(
  ...
  async getState(id, restAuth) {
    let data = [];
    let response = await fetch(`/api/dash?id=${id}`).catch((error) => {
      restAuth.putLog(
  ...
  async getSvg(svg, restAuth) {
    let data = [];
    let response = await fetch(`/svg/${svg}`).catch((error) => {
      restAuth.putLog(
  ...
  async setSvg(svg, restAuth) {
    ...
    let response = await fetch(`/api/load/svg`, {
      // сперва нужно подать post запрос
      method: 'POST',
      body: svg,
      // mode: 'no-cors'
  ...
  async importDash(dash, restAuth) {
    ...
    let response = await fetch(`/api/${dash.element}/import`, {
      // сперва нужно подать post запрос
      method: 'POST',
      body: dash.formData,
      // mode: 'no-cors'
  ...
  async getThemeBack(restAuth) {
    let data = [];
    let response = await fetch(`/api/user/setting`).catch((error) => {
      restAuth.putLog(
  ...
  async loadPaper(paper, restAuth) {
    ...
    let response = await fetch(`/api/eva/reports/load`, {
      // сперва нужно подать post запрос
      method: 'POST',
      body: paper,
      // mode: 'no-cors'
  ...
  async getAllPaper(restAuth) {
    let data = [];
    let response = await fetch(`/api/eva/reports/getAll`).catch((error) => {
      restAuth.putLog(
