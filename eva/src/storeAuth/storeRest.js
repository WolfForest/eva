export default {
    ...
    async setEssence(data) {
        ...
        // formData.append('id', this.userFrom.id);
        // formData.append('username', this.userData.username );
        // formData.append('password', this.userData.pass);
        // if (this.changedData) {
        //     let essence = this.changedData[this.essence[this.keyFrom-1]];
        //     Object.keys(essence).forEach( item => {
        //         formData.append(item,essence[item])
        //     })
        // }
        ...
        // if (response.status == 200 && data.save) {
        //     let alternative = response;
        //     alternative.json().then( res => {
        //         let name = res.name[0].toUpperCase() + res.name.slice(1);
        //         this.putLog(`Сохранен дашборд  ${name} c id ${data.id}`); 
        //     })
        //     //this.putLog(`Сохранен дашборд  ${response.status}`);
        // }
