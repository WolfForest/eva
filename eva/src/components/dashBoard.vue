  ...
  >
    <v-card
      ...
      :style="{
        ...
        boxShadow: `0 3px 1px -2px ${color.border},0 2px 2px 0 ${color.border},0 1px 5px 0 ${color.border}`,
      }"
    >
      <v-card-title v-show="props.disappear" class="card-title open_title">
        <div class="name-dash">
          <v-icon v-show="dataMode" class="icon chart" :color="color.controls">
...
export default {
  ...
  asyncComputed: {
    async prepareData() {
      ...
      if (this.element.includes("table") && this.shouldGet) {
        //данные с реста
        let search = this.$store.getters.getSearch({
          idDash: this.idDash,
          id: this.element,
        });
        let result = await this.startSearch(search);
        if (result.length > 0) {
          /// результат есть
          ...
          //результат не найден
          if (this.lastResult) {
            //показывать прошлый результат для элемента
            let searchId = this.$store.getters.getSearchID({
              idDash: this.idDash,
              id: this.element,
            });
  ...
  computed: {
    ...
    loadingSearch: function () {
      this.setLoading(
        this.$store.getters.getLoading(this.element, this.idDash)
      );
    ...
    shouldGet: function () {
      ...
      let should = false;
      if (this.element) {
        should = this.$store.getters.getShouldGet({
          id: this.element,
          idDash: this.idDash,
        });
      }
    ...
    lastResult() {
      //показывать прошлый релультат
      let lastResult = false;
      if (this.element) {
        lastResult = this.$store.getters.getShouldGet({
          id: this.element,
          idDash: this.idDash,
        });
      }
  ...
  methods: {
    ...
    checkFilter: function () {
        ...
        if (event.prop == "filter" && event.value == "true") {
          ...
          if (this.props.dataRestFilter.length == 0) {
            ...
            data.forEach((itemData) => {
              ...
              this.props.dataRestFilter.forEach((itemDataRest) => {
      ...
      if (data.length == 0) {
        this.props.dataRestFilter = JSON.parse(
          JSON.stringify(this.props.dataRest)
        );
      }
    },
