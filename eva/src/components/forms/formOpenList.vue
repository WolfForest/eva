           ...
           <v-container class="main-form-container" >
               <v-skeleton-loader  type="image" v-if="loading" height="300" width="100%"></v-skeleton-loader>
                <div class="main-form-lines" >
                    <v-card tile v-for="item in forms" :key="item[0]" class="form-line"  @click="openForm(item,$event)">
                        <v-list-item-group>
                            <v-list-item>
                                <div class="loading-bar " ></div>
                                <v-list-item-content  class="list-content">
                                    <v-list-item-title class="list-title">{{item[1].toUpperCase()}}</v-list-item-title>
                                    <v-list-item-subtitle class="list-subtitle">{{nameForm}}</v-list-item-subtitle>
                                </v-list-item-content>
                            </v-list-item>
                        </v-list-item-group>
                    </v-card>
...
export default {
    data () {
     ...
     computed: {  
        idForm: function() {  // id шаблона
             return this.$route.params.id
         },
         nameForm: function() { // имя шаблона
             return this.$route.query.name
         },
     },  
     methods: {
        getForms: async function(id) {  // получаем список всех форм данного шаблона
        ...
        getTemplate: async function(idForm) {  // получаем структуру шаблона
           let form =  await this.$store.form.getters.getTemplate(idForm);
           return form
        },
        getAllContent: async function(ids) {  // получаем контент конкретной формы
           let form =  await this.$store.form.getters.getFormContent(ids);
           return form
        },
        openForm: async function(name,event) {  // открываем уже существующию форму
           let element = event.target.parentElement; // получаем родителя элемента на который щелкнули
           while(!element.classList.contains('v-list-item')){  // и пока не "всплывем" до нужного нам родителя
           ...
           let content = await this.getAllContent({idTemplate:this.idForm, idForm:name[0]}); // получаем контент из базы данных
           ...
           content.forEach( (item,i) => { // пробегаемся по всему массиву
               if (Number(item)) {
           ...
           this.$router.push(`/forms/edit?editable=false&id=${this.idForm}&nameForm=${name[1]}`);   // переходим на страницу редактирования формы
        },
        createFormTemplate: async function(event) {  // мтеод создающий новую форму
            ...
            this.$router.push(`/forms/edit?editable=true&id=${this.idForm}&nameForm=Новая форма`);  // и переходим на страницу редактирования
