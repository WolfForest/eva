<template >
    <v-dialog  v-model="modal"  width="550" persistent  > 
       <!--  <div class="create-modal-block" :class={create_warning:create_warning} > -->
        ...
        <v-card >
            <v-card-text class="headline "><div class="choose-title">Выберите тип ячейки</div></v-card-text>
            <div class="elements-block" ref="elemBlock">
                <div class="element" v-for="element in elements" :key="element.name" @click="selectElem($event)">
...
import {   mdiFormatTextVariant, mdiNumeric, mdiFormatListBulleted,  mdiRadioboxMarked } from '@mdi/js'
...
export default {
    ...
    data () {
     ...
     methods: {
        ...
        selectElem: function(event) {  // функция для выбора одного элемента из списка
                ...
                this.$refs.elemBlock.childNodes.forEach( item => {  // пробегаемся по всем элементам
