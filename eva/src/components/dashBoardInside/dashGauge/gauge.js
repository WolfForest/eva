      ...
      for (j = 0, len = ref.length; j < len; j++) {
        ...
        if (value.label !== void 0) {
          ...
          if ((!this.options.limitMin || value >= this.minValue) && (!this.options.limitMax || value <= this.maxValue)) {
            ...
            const progressiveMinDistance = 0.032 * labelText.length;
            const shiftText = j > 0 && (rotationAngle - (this.getAngle(ref[j-1]) - 3 * Math.PI / 2)) < progressiveMinDistance;
            if (shiftText) {
              line++;
              this.ctx.translate(0, -lineHeight*line);
            } else {
              line = 0
            }

            this.ctx.fillText(labelText, 0, -radius - this.lineWidth / 2);
