<template  >
    <div class="dash-graph-block" :data-changeData="dataGet"    >
        <div class="graph-block-itself" v-show="!nomsg">
            <div ref = "graph" class="dash-graphVis" ></div>
            <v-tooltip bottom :color="colorFrom.controlsActive"   >
                <template v-slot:activator="{ on }">
                    <v-icon  class="change-direct" :color="colorFrom.controls" v-show="!errorData" v-on="on"  @click="changeDirect">{{changeIcon}}</v-icon>
                </template>
                <span>Повернуть граф</span>
            </v-tooltip>
            <div class="tooltipName" ref = 'tooltipName' :style="{background:colorFrom.controlsActive}" v-html="tooltipName"></div>
            <div class="changeName" ref = 'changeName' :class="{showchange:showChangeName}" :style="{border: `2px solid ${colorFrom.controlsActive}`,background:colorFrom.back}">
                <v-text-field :color="colorFrom.text" class="textarea-item" solo flat clearable  v-model="newNameRect" hide-details></v-text-field>
                <div class="accept-btn" :style="{background: colorFrom.controlsActive}" @click="setName">
                    <v-icon  class="save"    >{{saveicon}}</v-icon>
                </div>
            </div>
            <div class="saveTree" ref = 'saveTree' @click="saveTree" :style="{background:colorFrom.controlsActive}">Сохранить</div>
        </div>
        <div class="nodata" v-show="nomsg"> {{ nomsgtext }}</div>
    </div>
</template>
...
export default {
    ...
    data () {
     ...
     computed: {
        ...
        //     if(elem && this.preperedData.links) {  // если он уже создался на странице
        //     console.log('color');
     ...
     methods: {
         createGraph: function(elem) {  // функция отрисовки графа
                ...
                let paper = new joint.dia.Paper({   // объект полотна для графа
                    ...
                    paper.on('element:pointerclick', element => {
                           // let name = element.$el[0].textContent;
                           this.setClick(element.el.querySelector('text').getAttribute('data-name'));
                            // if (name[name.length-1] == '.') {
                            //     name = name.slice(0, -3);
                            // }
                            // Object.keys(this.labels).forEach( item => {
                            //     console.log(name,item,item.indexOf(name));
                ...
                Object.keys(tree[this.direct]).forEach((item, i) => { // для каждого узла из дерева элементов
          ...
          //   console.log(tree);
           ...
           //   console.log(links);
                ...
                links.forEach((item, i) => {  // пробегаемся по массиву линий
                      ...
                      // console.log(itemChild);
                       ...
                       // console.log(tree[this.direct]);
                        if (tree[this.direct][itemChild][2] < tree[this.direct][item.from][2]) { // если потомок находиться на уровень выше чем предок (если то откуда должна идит стрелка находиться ниже чем куда должна идти)
                         ...
                         //link.attr('line/stroke',this.colorFrom.text); // задаем текст стрелки
                ...
                function paintNode(node,colorFrom) {   // функция которая понимает стоит ли закрашивать узел одним цветом или двумя
                        ...
                        // Object.values(nodes).forEach( item=> {  // пробегаемся по массиву всех узлов
                        //         if (item.node == node) {  // если нашли нужный узел
                        ...
                        //     Object.values(nodes).forEach( item=> {  // пробегаемся по массиву всех узлов
                        //         if (item.node == node) {  // если нашли нужный узел
                        //             if (parseInt(item.err) == 1) {  // смотрим какое значение пришло в ошибках
                        //                 color = '#FF3333';  // красим в красный 
                        //             } else {   // или 
                        //                 color = '#008000';  // в зеленый 
                        //             }
                        //         }
                        // });
                        // rect.attr('left/fill', color);  // красим либо в один цвет либо другой
                ...
                function linkOptions(label,colorFrom) {  // функция для катсоматизации подписи на самих стрелках, в данный момнет не нужна.
                        ...
                        return [{
                            ...
                            attrs: {
                                ...
                                // labelBody: {   // стили background подписи
                                //     ref: 'labelText',
                                //     refX: 0,
                                //     refY: 0,
                                //     refWidth: "100%",
         ...
         changeName: function(element,event,x,y) {
             let name = '';
         ...
         sizeGraph: function() {  // получить размеры контейнера для графа
            ...
            //  } else {
            //     size = this.$refs.graph.parentElement.parentElement.parentElement.getBoundingClientRect(); // получаем размеры контейнера поднимаясь на пару уровней выше
            //     if (size.width > 0 && size.height> 0) { // если он уже отрисован и размеры есть
            //         return {'width': size.width-10, 'height':size.height-50};  // то добовляем их в объект
            //     }
            //  }
        ...
        prepareGraph: function(data) {  // первичная подготовка данных
            ...
            if (!data[0].relations || !data[0].node) {
                let elem = this.$refs.graph; // получаем элемент графа
               // elem.innerHTML = `<p class="errormsg" v-show="errorData" :color="this.colorFrom.text">Невозможно построить граф по этим данным</p>`;
                this.nomsg = true;
        ...
        //     console.log(data);
                    ...
                    data.forEach((item, i) => { // пробегаемся по всем данным
         ...
         setSizeElements: function(result,elementSizes,element) {  // сохраняем размер элемнета
             if (element == 'nodes'){  // если это нода
                ...
                result.nodes.forEach( (item,i) => { // пробегаемся по всем нодам
         ...
         createTree: function() {   // создаем дерево с позициями нод относительно хоста
             ...
             if(levels != 0){  // если уровни релаьно есть
                // console.log(this.structure);
                 ...
                 let step, otstup, center, odd, label;  // несоклько переменных
         ...
         stepByStep:  function(data) { // функция выоплняющяя все шаги по созаднию графа
                //  console.log(this.dataRest.length);
   ...
   mounted() {      // при первом появлении графа
       ...
       // setTimeout( ()=> {  // делаем трюк с задержкой в ноль секунд, чтобы он отработал после отрисовки других компонентов
            let stateTree =  this.$store.getters.getGraphTree({idDash: this.idDash, id: this.id });  // забираем уже сохраненные данные из store
            // Object.keys(stateTree.tree).forEach( item => {
            //     this.tree[item] = stateTree.tree[item];
            // })
            this.tree = stateTree.tree; // добовляем сохраненный граф
           // this.tree = Object.assign({},stateTree.tree);
         ...
         //   console.log('mount');
           // this.stepByStep(this.dataRest);
         //   this.stepByStep();  // функция отрисовывающяя граф по шагам
       // },0)
