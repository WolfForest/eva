    ...
    methods: {
      ...
      addSearch: function () {
        ...
        searches.forEach((item, i) => {
          ...
          if (item.sid == this.newSearch.sid) {
        ...
        if (j != -1) {
          ...
          this.$el.querySelector('.warning-block').style = `bottom: -${
            height + 55
          }px; opacity: 1; z-index: 3; visibility: visible`; // 45 это высота самого warning и padding сверху
          this.msgWarn = 'Такой источник данных существует. Хотите обновить?';
          this.$el
            .querySelector('.warning-block')
            .querySelector('.yes-btn')
            .setAttribute('tool', 'search');
        } else {
          // если нету
