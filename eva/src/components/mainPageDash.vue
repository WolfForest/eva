<template>
     <v-app >
      <header-top @permissions="setPermissions"></header-top>
                    ...
                    <v-tabs   v-model="tab" class="tab-dash" :color='color.controls' >
                        ...
                        <v-tab-item :value="'tab-1'" class="groups-of-dash">
                                <v-card  class="dash-group"    :ripple="false"  v-for="i in allGroups.length" :key="i" >   
                                    ...
                                    <v-card-text class="dash-group-text" @click="getDash({id:allGroups[i-1].id,color:allGroups[i-1].color,name: allGroups[i-1].name})">
                                        <p class="group-text">{{checkName(allGroups[i-1].name)}}</p>
                                    </v-card-text>
                                </v-card>
                                <v-btn
                        ...
                        <v-tab-item :value="'tab-2'" class="groups-of-dash" >
                            <v-card  class="dash-group"    :ripple="false"  v-for="i in allDashs.length" :key="i">   
                                ...
                                <v-card-text class="dash-group-text" @click="goToDash(i-1)">
                                    <p class="group-text">{{checkName(allDashs[i-1].name)}}</p>
                                </v-card-text>
                            </v-card>
                             <v-btn
                                    :color='color.controls'
       ...
       <modal-exim :color="color" :active="modalExim"  :curName="curName" :dashboards="allDashs" :groups="allGroups" :element="element" @closeModal="closeModal"></modal-exim>
       <modal-create :modalFrom="modalCreateGroup" :permissionsFrom="adminRoot" :actionFrom="actionBtn" :groupFrom="allGroups" :nameGroupFrom="cookieName" :dashsFrom="allDashs" :dataFrom="allGroups[curGroup]" :dashFrom="allDashs[curGroup]" :curGroupFrom="curGroup" :groupFlagFrom="createGroupFlag" @createGroup="createGroup($event)"  :colorFrom="color" @closeModal="closeModal"></modal-create>
       <modal-delete-main :modalFrom="modalDelete" :nameFrom="nameDelete" @deleteElem="deleteElem" :colorFrom="color"  @closeModal="modalDelete=false"  ></modal-delete-main>
   </v-app> 
</template>
...
export default {
    ...
    //   asyncComputed: {
    //     async groups() {
    //         this.allGroups  =   await this.getGroups();
    //         return this.allGroups 
    //     },
    //   },
      computed: {
        //   dashs: function() {
        //        this.allDashs = this.$store.getters.getDashs;
        //        if (Object.keys(this.allDashs).length == 0) {
        //           // здесь будем отправлять на сервер
        //           this.allDashs = [{name:'Dash-1',id:'dash-1'},{name:'Dash-2',id:'dash-2'}];
        //           this.$store.commit('setDashs',this.allDashs);
        //        } 
      ...
      methods: {
        ...
        // createGroup: function(group,method) {
        //     // let i = -1;
        //     // this.allGroups.forEach( (item,j) => {
        ...
        //     // if (i != -1) {
        //     //    this.allGroups[i] = group 
        //     // } else {
        //     //    this.allGroups.push(group);
        //     //    this.$store.auth.getters.putLog(`Создана группа ${this.toHichName(group.name)}`);
        //     // }
        //     console.log(method)
        //     let response = this.$store.auth.getters.setEssence({formData: JSON.stringify(group), essence: 'group',method: method});
        //      response.then( res => {
        //         if (res.status == 200){
        //             this.cancelModal();
        ...
        deleteElem: function() {
            ...
            //let essence = -1;
           // this.elemDelete == 'group' ? essence = 4 : essence = 6;
           let response = null;
           let data = null;
           let id = -1;
