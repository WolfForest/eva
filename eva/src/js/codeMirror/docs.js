const docs = {
  entityFunctions: {
    ...
    appendpipe: `
        ...
        <div>
          КомандаКоманда добавляет дополнительные преобразования (subpipeline) к результатам 
          основного запроса. В отличие от подзапроса (subsearch), subpipeline не запускается первым. 
          Subpipeline запускается тогда, когда запрос достигает команды <b>appendpipe</b>.
        </div>
        <div>Обязательный параметр:</div>
        <ul>
    ...
    chart: `
        ...
        <div>Команда <b>chart</b> группирует записи по указанному полю и является трансформирующей командой,
         ...
         <br>
         В команде <b>chart</b> должна быть определена статистическая функция для агрегации данных.
        </div>
        <div>Обязательный параметр:</div>
        <ul>
    ...
    collect: `
        ...
        <div>Обязательный параметр:</div>
        <ul>
        <li><b>index</b> - имя индекса, в который добавляются события.</li></ul>
        <div>Опциональные параметры:</div>
        <ul>
    ...
    dedup: `
        ...
        <div>Команда удаляет события, которые содержат повторяющееся значение поля
         ...
         необходимо предусмотреть сортировку событий до их поступления на вход команды <b>dedup</b>.
         Для этого, например, можно использовать команду <b>sort</b>.
        </div>
        <div>Обязательный параметр:</div>
        <ul>
    ...
    delta: `
        ...
        <div>Вычисляет разницу между соседними результатами, используя значение определенного числового поля.
         ...
         необходимо предусмотреть сортировку событий внутри запроса (использование команды <b>sort</b>).
        </div>
        <div>Обязательный параметр:</div>
        <ul>
    ...
    eval: `
        ...
        <div>Команда <b>eval</b> вычисляет выражение и помещает полученное значение в поле результатов поиска:
          ...
          Команда <b>eval</b> оценивает математические, строковые и логические выражения.
    ...
    fields: `
        ...
        <ul>
        <li>
          <b>field-list</b> - список полей (через запятую), к которым должна быть применена команда <b>fields</b>.
    ...
    filldown: `
        ...
        <div>Если значение поля NULL, то команда заполняет поле ближайшим сверху значением этого же 
        поля, не равным NULL. Если у поля не было предыдущих не-NULL значений, то после применения 
        <b>filldown</b> поле остаётся пустым (NULL)..
        </div>
        <div>Опциональные параметры:</div>
        <ul>
    ...
    fillnull: `
        ...
        <li><b>field-list</b> - одно или несколько полей, разделенных пробелами.
         ...
         Синтаксис: <i>&lt;field1&gt;, &lt;field2&gt;, ...</i>
        </li>
        <li><b>value=&lt;string&gt;</b> - определяет, чем будут заполнены NULL-значения. Значение по умолчанию – 0.
        </li>
        </ul>
      `,
    foreach: `
    ...
    head: `
        ...
        <div>Команда возвращает первые N результатов поиска. Отсчет результатов будет вестись в порядке поиска,
         то есть, в том порядке, в котором события поступают на вход команды <b>head</b> после обработки
         ...
         событий до их поступления на вход команды <b>head</b>. Для этого можно использовать команду <b>sort</b>.
        </div>
        <div>Опциональные параметры:</div>
        <ul>
    ...
    inputlookup: `
        ...
        <div>Команда <b>inputlookup</b> используется для вывода содержимого lookup-таблицы
         ...
         Для получения корректного результата поиска по датам и времени используйте команды <b>otstats</b> и <b>search</b>.
        </div>
        <br>
        <div>Обязательный параметр</div>
        <ul>
    ...
    join: `
        ...
        <div>Команда <b>join</b> комбинирует результаты поискового подзапроса с результатами основного запроса.
         ...
         Альтернативные команды. Если для решения задачи не требуется в точности семантика <b>join</b>,
         то использование схожих по функционалу команд может быть более целесообразно:
         <b>append, appendcols, lookup, search, stats</b>.
        </div>
        <div>Обязательный параметр</div>
        <ul>
        <li><b>subsearch</b> – опции команды <b>join</b>. <br>
          ...
          <li><b>field-list</b> - поля, по которым будет выполняться команда <b>join</b>. Совпадение значений
    ...
    makeresults: `
        ...
        <div>Команда создает (генерирует) требуемое количество результатов поиска.
          ...
          <b>Важно!</b><br>
          Эту команду не рекомендуется применять при поиске по датам и времени.
          Для получения корректного результата поиска по датам и времени пользуйтесь командами <b>otstats</b> и <b>search</b>.
        </div>
        <div>Опциональные параметры:</div>
        <ul>
    ...
    nomv: `
        <h3>nomv &lt;field&gt;</h3>
        <div>Команда конвертирует значения multivalue-поля в singlevalue-поле.
        </div>
        <div>Обязательный параметр:</div>
        <ul>
        <li>
    ...
    otloadjob: `
        ...
        <li>
          <b>spl-query</b> – поисковый запрос, результаты которого требуется извлечь из кеша.
           Успешное выполнение команды <b>otloadjob</b> возможно только при наличии двух условий:
           <ul>
    ...
    outputlookup: `
        ...
        <li>
          <b>append</b>:
          <ul>
          <li>если <b>append</b>=false (по умолчанию), то команда <b>outputlookup</b> перезаписывает данные в существующем lookup-файле;</li>
    ...
    rangemap: `
        ...
        <li>
          ...
          <ul>
          <li><b>attribute_name</b> - строка, выводимая в поле range, когда значение в <b>field</b> попадает в диапазон значений из <b>numeric_range</b>.</li>
          <li><b>numeric_range</b> - начальное и конечное значения диапазона, заданные через слеш (-).</li>
          </ul>
    ...
    sort: `
        ...
        <div>Команда <b>sort</b> сортирует результаты по указанному полю. События, в которых поле сортировки отсутствует,
    ...
    spath: `
        ...
        <div>Команда <b>spath</b> позволяет извлекать информацию из структурированных данных в формате JSON.
        ...
        <div>Опциональный параметр:</div>
        <ul>
        <li><b>input</b> - поле для чтения и извлечения значений. Значение по умолчанию: _raw.</li>
    ...
    stats: `
        ...
        <li><b>AS &lt;field&gt;</b> - имя поля, в которое будет записан результат статистической функции.
         По умолчанию, если не указывается <b>AS &lt;field&gt;</b>, то именем поля будет сама применяемая функция.</li>
    ...
    streamstats: `
        ...
        Команда <b>streamstats</b> считает статистику от события к событию, формируя каждый раз новый результат в момент
    ...
    superjoin: `
        ...
        <div>Команда <b>superjoin</b> комбинирует данные из таблицы, указанной в параметрах команды <b>superjoin</b>,
         с результатами основного запроса без каких-либо ограничений на размер таблицы.<br>
        <b>Важно!</b><br>
        Команда <b>superjoin</b> не является частью стандартного набора команд. Находится в плагине OTLExtend.
        </div>
        <div>Обязательный параметр:</div>
        <ul>
        <li>
        ...
        <li>
          <b>superjoin-params</b> – опции команды <b>superjoin</b>. <br>
